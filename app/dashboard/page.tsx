...
export default function Dashboard() {
  ...
  const fetchFeedbacks = async () => {
    ...
    setError("");
    try {
      const res = await fetch("http://localhost:3005/api/feedback/home", {
        method: "GET",
        headers: {
  ...
  const handleLike = async (id: string) => {
    try {
      const res = await fetch(
        `http://localhost:3005/api/feedback/likefeedbacks/${id}/like`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:3005/api/feedback/delete/${id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
      ...
      if (res.ok) {
        ...
        console.log(`Feedback ${id} deletado:`, result);
        setFeedbacks((prevFeedbacks) =>
